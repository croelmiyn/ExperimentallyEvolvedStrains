...
public class SimuIOR12DA {
    ...
    void runInitGUI(){
        canceled = SimulationBoxPropertiesR12DA.runInitGUI();
        canceled = CellPropertiesR12DA.setPropertiesGUI();
        if(!canceled) {
            setSaveNames();
        }
    }
    ...
    void saveDATA(ArrayList<TrajectoryR12DA> trajs){

        if(saveNameDATA==null){
            return;
        }

        TrajectoryR12DA traj;
        ...
        try {
            ...
            dos.writeInt(nData);
            dos.writeInt(SimulationBoxPropertiesR12DA.nFrames);
            dos.writeInt(nTraj);
    ...
    ArrayList<TrajectoryR12DA> readDATA(String fileName){

        ArrayList<TrajectoryR12DA> trajs = new ArrayList<>();
        ...
        try{
            ...
            for(int i=0; i<nData; i++){
                ...
                if(tn!=curT){
                    curT = tn;
                    TrajectoryR12DA traj = new TrajectoryR12DA(tn, new CellR12DA(fn,x,y,phi,Math.sqrt(vx*vx+vy*vy),new CellPathwayR12DA(n0,k0,m0)));
                    trajs.add(traj);
                }
                else {
    ...
    void saveLOG(){
        ...
        buffer += SimulationBoxPropertiesR12DA.getParameterLog();

        buffer += CellPropertiesR12DA.getPropertiesLog();
    ...
    void generateMovie(ArrayList<TrajectoryR12DA> trajs){

        if(movieName==null || movieType.equals("none")){
            return;
        }

        imp = IJ.createImage(movieName,movieType, SimulationBoxPropertiesR12DA.frameSizeX, SimulationBoxPropertiesR12DA.frameSizeY, SimulationBoxPropertiesR12DA.nFrames);
        float[][] zeros = new float[SimulationBoxPropertiesR12DA.frameSizeX][SimulationBoxPropertiesR12DA.frameSizeX];
        for(int i = 0; i< SimulationBoxPropertiesR12DA.nFrames; i++){
            imp.setSlice(i+1);
        ...
        ParallelUtil.parfor(trajs.size(),"printTraj",(k)->{
            ...
            for(int i=0;i<t.getSize();i++){
                ...
                x_per = t.x.get(i) - SimulationBoxPropertiesR12DA.frameSizeX * Math.floor(t.x.get(i)/ SimulationBoxPropertiesR12DA.frameSizeX);
                y_per = t.y.get(i) - SimulationBoxPropertiesR12DA.frameSizeY * Math.floor(t.y.get(i)/ SimulationBoxPropertiesR12DA.frameSizeY);
                ...
                for(int xx=-(int)(L+e); xx<=(L+e); xx++){
                    xl = (x0+xx)% SimulationBoxPropertiesR12DA.frameSizeX;
                    for(int yy=-(int)(L+e); yy<=(L+e); yy++){
                        yl = (yy+y0)% SimulationBoxPropertiesR12DA.frameSizeY;
